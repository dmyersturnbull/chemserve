...
from chemgrid.models import BaseChem, Concrete
...
class ChemGrid:
    ...
    @classmethod
    def simple_server(cls, fn: Callable[[Concrete, Mapping[str, Any]], Any], port: int, return_hash: bool = False) -> Server:
        def receiver(go: Json):
            payload = ConcretePayload.decode(go)
            values = {}
            for compound, item in zip(go['compounds'], payload.items):
...
__all__ = ['Client', 'Server', 'ChemGrid']
